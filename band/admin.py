...
class BandPostImageInline(TabularInline):
    model = BandPostImage
    extra = 1


@admin.register(BandPost)
class BandPostAdmin(ModelAdmin):
...
class BandCommentAdmin(ModelAdmin):
...
class BandPostLikeAdmin(ModelAdmin):
...
class BandCommentLikeAdmin(ModelAdmin):
    list_display = ["comment", "user", "created_at"]
    list_filter = ["created_at"]
    search_fields = ["comment__content", "user__activity_name"]


class BandVoteOptionInline(TabularInline):
    model = BandVoteOption
    extra = 2


@admin.register(BandVote)
class BandVoteAdmin(ModelAdmin):
...
class BandScheduleAdmin(ModelAdmin):
...
class BandScheduleApplicationAdmin(ModelAdmin):
...
class BandBookmarkAdmin(ModelAdmin):
