...
function ParticipantScreen({ state, actions }) {
  ...
  const myCourt = state.courts.find((c) => c.match && [...c.match.teamA, ...c.match.teamB].some((p) => p.id === me.id));
  const inNext0 = nextGroups[0] && [...nextGroups[0].teamA, ...nextGroups[0].teamB].some((p) => p.id === me.id);
  const inLater = !inNext0 && nextGroups.slice(1).some((g) => [...g.teamA, ...g.teamB].some((p) => p.id === me.id));
  const qpos = queue.findIndex((p) => p.id === me.id);
  const isCoach = coachIds.has(me.id);
  const myCoachCourt = isCoach ? state.courts.find((c) => c.ace && c.coachId === me.id) : null;
  const coachCourtName = myCoachCourt ? (myCoachCourt.name || myCoachCourt.no + '번 코트') : '';

  // 파트너: 승인된 쌍 / 승인 대기 신청
  const nameById = (pid) => (state.participants.find((p) => p.id === pid) || {}).name;
  const myPair = (state.pairs || []).find((pr) => pr.members.includes(me.id));
  const mateName = myPair ? nameById(myPair.members.find((m) => m !== me.id)) : '';
  const myReq = (state.pairRequests || []).find((r) => r.from === me.id || r.to === me.id);
  const reqMateName = myReq ? nameById(myReq.from === me.id ? myReq.to : myReq.from) : '';
  const inUse = (pid) => (state.pairs || []).some((pr) => pr.members.includes(pid)) || (state.pairRequests || []).some((r) => r.from === pid || r.to === pid);
  const partnerCands = present.filter((p) => p.id !== me.id && !coachIds.has(p.id) && !inUse(p.id));
      ...
      <div style={{ width: 400, maxWidth: '100%', padding: '0 16px 28px', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ background: 'var(--surface)', borderRadius: 20, border: '1px solid var(--line)', boxShadow: 'var(--sh-2)', padding: 20, marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 13 }}>
            <LevelChip level={me.level} size={42} />
            <div style={{ flex: 1, minWidth: 0 }}>
              <NameWithGender p={me} size={20} />
              <div style={{ fontSize: 12.5, color: 'var(--muted)', fontWeight: 600, marginTop: 3 }}>오늘 {me.totalGames}경기{restMin != null ? ` · ${restMin}분 휴식` : ''}</div>
            </div>
            <span style={{ fontSize: 12, fontWeight: 800, color: accent, background: tint, padding: '5px 11px', borderRadius: 999, whiteSpace: 'nowrap' }}>{badge}</span>
          </div>
          <div style={{ marginTop: 16, padding: '14px 16px', borderRadius: 14, background: tint }}>
            <div style={{ fontSize: 17, fontWeight: 900, color: accent, letterSpacing: '-.02em' }}>{title}</div>
            {sub && <div style={{ fontSize: 13, color: 'var(--ink-2)', fontWeight: 500, marginTop: 4, lineHeight: 1.5 }}>{sub}</div>}
          </div>
        </div>

        {/* 함께 칠 멤버 */}
        {myMatch && (
          <div style={{ background: 'var(--surface)', borderRadius: 18, border: '1px solid var(--line)', boxShadow: 'var(--sh-1)', padding: 16, marginBottom: 12 }}>
            <div style={{ fontSize: 12.5, fontWeight: 800, color: 'var(--muted)', marginBottom: 11 }}>{myCourt ? '이번 경기' : '다음 경기'} · {myMatch.type}</div>
            <div style={{ display: 'flex', alignItems: 'stretch', gap: 8 }}>
              {[myMatch.teamA, myMatch.teamB].map((team, ti) => (
                <React.Fragment key={ti}>
                  {ti === 1 && <div style={{ alignSelf: 'center', fontSize: 11, fontWeight: 900, color: 'var(--muted)' }}>VS</div>}
                  <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {team.map((p) => (
                      <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '7px 9px', borderRadius: 10, background: p.id === me.id ? 'var(--brand-tint)' : 'var(--surface-2)', border: p.id === me.id ? '1px solid var(--brand)' : '1px solid transparent' }}>
                        <LevelChip level={p.level} size={20} />
                        <NameWithGender p={p} size={13.5} />
                        {p.id === me.id && <span style={{ fontSize: 11, fontWeight: 800, color: 'var(--brand-ink)', marginLeft: 1 }}>(나)</span>}
                      </div>
                    ))}
                  </div>
                </React.Fragment>
        ...
        {/* 파트너 게임 신청 (바텀시트로 선택 → 모임장 승인) */}
        {myPair ? (
          <div style={{ background: 'var(--surface)', borderRadius: 16, border: '1px solid var(--brand)', padding: '13px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 13.5, fontWeight: 800, color: 'var(--brand-ink)' }}>{mateName} 님과 파트너 ✓</span>
            <button onClick={() => actions.removePair(myPair.id)} style={{ marginLeft: 'auto', padding: '7px 13px', borderRadius: 9, background: 'var(--surface)', border: '1px solid var(--line-2)', color: 'var(--muted)', fontSize: 12.5, fontWeight: 800, cursor: 'pointer' }}>해제</button>
          </div>
        ) : myReq ? (
          <div style={{ background: 'var(--warn-tint)', borderRadius: 16, border: '1px solid var(--warn)', padding: '13px 16px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{ fontSize: 13, fontWeight: 800, color: 'var(--warn)' }}>{reqMateName} 님과 파트너 신청함 · 모임장 승인 대기</span>
            <button onClick={() => actions.rejectPairRequest(myReq.id)} style={{ marginLeft: 'auto', padding: '7px 13px', borderRadius: 9, background: 'var(--surface)', border: '1px solid var(--line-2)', color: 'var(--muted)', fontSize: 12.5, fontWeight: 800, cursor: 'pointer', flexShrink: 0 }}>취소</button>
          </div>
        ) : (
          <button onClick={() => setShowPartner(true)} style={{ width: '100%', padding: 14, borderRadius: 13, background: 'var(--surface)', border: '1px solid var(--line-2)', color: 'var(--ink)', fontSize: 15, fontWeight: 800, cursor: 'pointer', marginBottom: 12 }}>파트너 게임 신청하기</button>
        )}

        {/* 퇴장 / 복귀 */}
        {me.status === '참여중' ? (
          <button onClick={() => actions.setStatus(me.id, '퇴장')} style={{ width: '100%', padding: 15, borderRadius: 13, background: 'var(--surface)', border: '1px solid var(--line-2)', color: 'var(--danger)', fontSize: 15, fontWeight: 800, cursor: 'pointer' }}>퇴장하기</button>
        ) : (
          <button onClick={() => actions.setStatus(me.id, '참여중')} style={{ width: '100%', padding: 15, borderRadius: 13, background: 'var(--brand)', border: 'none', color: '#fff', fontSize: 15, fontWeight: 800, cursor: 'pointer' }}>복귀하기</button>
        )}
        <p style={{ textAlign: 'center', fontSize: 11.5, color: 'var(--muted)', margin: '10px 0 0', lineHeight: 1.5 }}>중간에 가셔도 [퇴장]만 누르면 대진에서 빠져요. 다시 오면 [복귀].</p>
      </div>

      {/* 파트너 선택 바텀시트 */}
      {showPartner && !myPair && (
        <div onClick={() => { setShowPartner(false); setPSel(''); setPq(''); }} style={{ position: 'absolute', inset: 0, zIndex: 50, background: 'rgba(17,22,31,.5)', display: 'flex', alignItems: 'flex-end', justifyContent: 'center', animation: 'fadeIn .18s ease' }}>
          <div onClick={(e) => e.stopPropagation()} style={{ width: 430, maxWidth: '100%', maxHeight: '84%', background: 'var(--surface)', borderRadius: '26px 26px 0 0', boxShadow: 'var(--sh-3)', display: 'flex', flexDirection: 'column', animation: 'sheetUp .3s cubic-bezier(.16,1,.3,1) both' }}>
            <div style={{ display: 'flex', justifyContent: 'center', paddingTop: 10 }}><div style={{ width: 42, height: 5, borderRadius: 5, background: 'var(--line-2)' }} /></div>
            <div style={{ padding: '12px 20px 10px' }}>
              <div style={{ fontSize: 19, fontWeight: 900, letterSpacing: '-.03em', color: 'var(--ink)' }}>함께 칠 파트너</div>
              <div style={{ fontSize: 12.5, color: 'var(--muted)', fontWeight: 600, marginTop: 3, lineHeight: 1.5 }}>같은 종목 경기에선 둘이 항상 한 팀으로 들어가요. 신청하면 <b style={{ color: 'var(--warn)' }}>모임장 승인</b> 후 적용되고, 종목·상대가 맞아야 들어가서 <b style={{ color: 'var(--warn)' }}>경기 수가 조금 줄 수 있어요.</b></div>
              <div style={{ position: 'relative', marginTop: 12, display: 'flex', alignItems: 'center' }}>
                <input value={pq} onChange={(e) => setPq(e.target.value)} placeholder="이름·급수로 검색" style={{ width: '100%', padding: '11px 14px', borderRadius: 11, border: '1px solid var(--line-2)', background: 'var(--surface-2)', fontSize: 14, fontWeight: 600, color: 'var(--ink)', outline: 'none', fontFamily: 'inherit' }} />
              </div>
            </div>
            <div style={{ flex: 1, overflowY: 'auto', padding: '2px 14px 8px' }}>
              {partnerCands.filter((p) => pq.trim() === '' || p.name.toLowerCase().includes(pq.trim().toLowerCase()) || (p.level && p.level.toLowerCase().includes(pq.trim().toLowerCase()))).map((p) => {
                const on = pSel === p.id;
                return (
                  <button key={p.id} onClick={() => setPSel(p.id)} style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 11, padding: '11px 12px', borderRadius: 13, marginBottom: 6, textAlign: 'left', background: on ? 'var(--brand-tint)' : 'var(--surface)', border: on ? '1px solid var(--brand)' : '1px solid var(--line)', cursor: 'pointer' }}>
                    <LevelChip level={p.level} size={28} />
                    <div style={{ flex: 1, minWidth: 0 }}><NameWithGender p={p} size={15.5} /></div>
                    <span style={{ width: 22, height: 22, borderRadius: 999, flexShrink: 0, border: on ? 'none' : '1.5px solid var(--line-2)', background: on ? 'var(--brand)' : 'transparent', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
                      {on && <svg width="13" height="13" viewBox="0 0 24 24" fill="none"><path d="M5 12.5l4.5 4.5L19 7" stroke="#fff" strokeWidth="3.2" strokeLinecap="round" strokeLinejoin="round" /></svg>}
                    </span>
                  </button>
                );
              })}
              {partnerCands.length === 0 && <div style={{ padding: 30, textAlign: 'center', color: 'var(--muted)', fontSize: 13.5 }}>신청할 수 있는 상대가 없어요.</div>}
            </div>
            <div style={{ padding: '12px 16px 18px', borderTop: '1px solid var(--line)', background: 'var(--surface)' }}>
              <button disabled={!pSel} onClick={() => { actions.requestPair(me.id, pSel); setShowPartner(false); setPSel(''); setPq(''); }} style={{ width: '100%', padding: 15, borderRadius: 13, background: pSel ? 'var(--brand)' : 'var(--surface-3)', color: pSel ? '#fff' : 'var(--muted)', border: 'none', fontSize: 15.5, fontWeight: 800, cursor: pSel ? 'pointer' : 'not-allowed' }}>{pSel ? '이 분과 파트너 신청' : '파트너를 선택하세요'}</button>
            </div>
          </div>
        </div>
      )}
...
Object.assign(window, { MainScreen, CourtCard, MiniCourt, ControlBar, QueuePanel, QueueNextBlock, QueueRow, CoachMark, ParticipantScreen });
