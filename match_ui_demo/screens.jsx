...
// ---------- 코트 카드 ----------
function CourtCard({ court, rec, nowTs, auto, onEnd, onStart, onEdit, onManualFill }) {
  const theme = useTheme();
  const clean = theme === 'clean';
  ...
  return (
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '12px 16px 8px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          {!playing && !type && <span style={{ fontSize: 13, fontWeight: 700, color: 'var(--muted)' }}>대기</span>}
        </div>
      </div>
      ...
      <div style={{ padding: '4px 14px 0', flex: 1 }}>
        {playing ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '28px 0', textAlign: 'center' }}>
            ...
            <div style={{ fontSize: 14, fontWeight: 700, color: 'var(--ink-2)' }}>{!auto ? '비어 있음' : (rec ? rec.error : '추천 대기')}</div>
            <div style={{ fontSize: 12.5, color: 'var(--muted)', maxWidth: 180 }}>{!auto ? '선수를 직접 넣어 주세요' : (rec ? rec.detail : '대기 인원이 모이면 추천됩니다.')}</div>
          </div>
        )}
      </div>
      ...
      <div style={{ padding: 14, display: 'flex', gap: 9 }}>
        {playing ? (
          <>
            ...
            <Btn variant="ghost" size="lg" onClick={() => onEdit(court)}>편집</Btn>
          </>
        ) : auto && rec && rec.match ? (
          <>
            ...
            <Btn variant="ghost" size="lg" onClick={() => onStart(court, rec.match, true)}>편집</Btn>
          </>
        ) : !auto ? (
          <>
  ...
  return (
    <div style={{
      ...
      <div data-help="rightnav" style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 16 }}>
        ...
        {stats.noShow > 0 && (
          <button onClick={onCheckin} style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '9px 14px', borderRadius: 10, fontSize: 14, fontWeight: 800, color: 'var(--warn)', background: 'var(--warn-tint)', letterSpacing: '-.02em', whiteSpace: 'nowrap' }}>
            체크인 <span style={{ background: 'var(--warn)', color: '#fff', borderRadius: 6, padding: '1px 6px', fontSize: 12.5 }}>{stats.noShow}</span>
          </button>
        )}
        <button onClick={() => onSwitch('attendance')} style={navBtnStyle}>출석</button>
...
// ---------- 대기열 패널 ----------
function QueuePanel({ queue, nextUp, nowTs, manual, onMakeGame, pending, onCancelPending }) {
...
function CoachMark({ auto, onClose }) {
  ...
  return (
    <div ref={overlayRef} onClick={onClose} style={{ position: 'absolute', inset: 0, zIndex: 60, cursor: 'pointer', animation: 'ovFade .25s ease both' }}>
      ...
      <div style={{ position: 'absolute', left: 0, right: 0, bottom: 138, display: 'flex', justifyContent: 'center', pointerEvents: 'none', animation: 'cardIn .4s ease .5s both' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 8, padding: '8px 16px', borderRadius: 999, background: 'rgba(21,23,28,.82)', color: '#fff', fontSize: 13, fontWeight: 700, letterSpacing: '-.02em', whiteSpace: 'nowrap' }}>
...
function MainScreen({ state, actions }) {
  ...
  const nextUp = useMemo(() => {
    ...
    return { groups, err };
  }, [pool, mode, preset, nowTs, state.auto]);
      ...
      <div style={{ flex: 1, display: 'flex', minHeight: 0 }}>
        <main data-help="court" style={{ flex: 1, overflowY: 'auto', padding: 20 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(330px, 1fr))', gap: 16 }}>
            {courts.map((ct) => (
              <CourtCard key={ct.no} court={ct} rec={recs[ct.no]} nowTs={nowTs} auto={state.auto} onEnd={actions.endMatch} onStart={actions.startMatch} onEdit={actions.openEdit} onManualFill={actions.manualFill} />
            ))}
          </div>
        </main>
        <QueuePanel queue={queue} nextUp={nextUp} nowTs={nowTs} manual={!state.auto} onMakeGame={actions.makeGameFromQueue} pending={state.pending} onCancelPending={actions.cancelPending} />
      </div>
